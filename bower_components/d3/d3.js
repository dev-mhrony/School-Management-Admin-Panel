!(function () {
          ...
          halfEdges.splice(
            ...
            new d3_geom_voronoiHalfEdge(
              d3_geom_voronoiCreateBorderEdge(
                ...
                abs(x3 - x0) < ε && y1 - y3 > ε
                  ? { x: x0, y: abs(x2 - x0) < ε ? y2 : y1 }
                  : abs(y3 - y1) < ε && x1 - x3 > ε
                  ? { x: abs(y2 - y1) < ε ? x2 : x1, y: y1 }
                  : abs(x3 - x1) < ε && y3 - y0 > ε
                  ? { x: x1, y: abs(x2 - x1) < ε ? y2 : y0 }
                  : abs(y3 - y0) < ε && x3 - x0 > ε
                  ? { x: abs(y2 - y0) < ε ? x2 : x0, y: y0 }
                  : null
              ),
              cell.site,
              null
  ...
  function d3_interpolateTranslate(ta, tb, s, q) {
    if (ta[0] !== tb[0] || ta[1] !== tb[1]) {
      ...
      q.push(
        { i: i - 4, x: d3_interpolateNumber(ta[0], tb[0]) },
        { i: i - 2, x: d3_interpolateNumber(ta[1], tb[1]) }
      );
    } else if (tb[0] || tb[1]) {
      s.push("translate(" + tb + ")");
    }
  ...
  function d3_interpolateScale(ka, kb, s, q) {
    if (ka[0] !== kb[0] || ka[1] !== kb[1]) {
      ...
      q.push(
        { i: i - 4, x: d3_interpolateNumber(ka[0], kb[0]) },
        { i: i - 2, x: d3_interpolateNumber(ka[1], kb[1]) }
      );
      ...
      var extent = d3_scaleExtent(scale.domain()),
        method =
          interval == null
            ...
            : !interval.range && [{ range: interval }, skip];
      if (method) (interval = method[0]), (skip = method[1]);
